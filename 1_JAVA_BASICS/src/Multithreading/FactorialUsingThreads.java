 ...
 *
 *  Since factorial of these numbers will be large, therefore we will store these numbers in data type BigInteger which has
 *  a larger range than int/integer.
 */
public class FactorialUsingThreads {
    ...
    public static void nonThreadedWork(){
        int[] arr = {
                10000, 20000 , 30000, 40000, 60000,50000,70000,90000,
                10000, 20000 , 30000, 40000, 60000,50000,70000,90000,
                10000, 20000 , 30000, 40000, 60000,50000,70000,90000
        };
        long start = System.currentTimeMillis();
        Arrays.stream(arr).forEach(x-> {
            FactorialUsingThreads.factorial(x);
        });
        long end = System.currentTimeMillis();
        System.out.println("Time taken to execute Non Threaded Work : " + (end - start)); // Output time = 15403
    }
    ...
    private static class myThread extends Thread{
        ...
        @Override
        public void run() {
            this.result = FactorialUsingThreads.factorial(num);
        }
    }
    ...
    public static void threadedWork() throws InterruptedException {
        ...
        for(int i=0;i<thread_arr.length;i++){
             ...
             * but this input/output work will not start until all the work in these threads is finished. Hence this creates a problem for a long process as main
             * thread gets blocked untill all the threads finishes the tasks. thought multiprocessing is happening here, but .join on thread will block the main
             * thread.
             */
            thread_arr[i].join();
    ...
    public static void main(String[] args) throws InterruptedException {
        nonThreadedWork(); // Time taken = 16895

        threadedWork(); // Time taken = 5036
    }
